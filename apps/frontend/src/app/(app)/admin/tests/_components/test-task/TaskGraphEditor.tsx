...
/**
 * Task graph editor — interactive SVG canvas for the optional graph
 * attached to a task. Behavior mirrors the lesson editor's
 * GraphBlockEditor (click-to-add vertex, click-two-to-connect, drag,
 * right-click-to-delete) and exposes a vertex-rename + edge-label list
 * underneath. Bound to `EditorTaskGraph` state owned by the parent
 * `TaskEditor`. The parent persists the graph row on save.
 */
...
interface Props {
  ...
  locale: 'en' | 'uk';
  onChange: (next: EditorTaskGraph) => void;
}

export function TaskGraphEditor({ graph, locale, onChange }: Props) {
  const { t } = useT();
  ...
  function emit(patch: Partial<EditorTaskGraph>) {
  ...
  function handleVertexClick(e: React.MouseEvent, id: string) {
    ...
    if (!selected) {
      ...
      const exists = edges.some(
        (ed: EditorGraphEdge) =>
          (ed.source === selected && ed.target === id) ||
  ...
  function handleVertexMouseDown(e: React.MouseEvent, id: string) {
    ...
    const { x, y } = svgXY(e);
    const v = vertices.find((v: EditorGraphVertex) => v.id === id)!;
    setDragging(id);
  ...
  function handleMouseMove(e: React.MouseEvent) {
    ...
    emit({ vertices: vertices.map((v: EditorGraphVertex) => v.id === dragging ? { ...v, x: nx, y: ny } : v) });
  }
  ...
  function handleVertexCtx(e: React.MouseEvent, id: string) {
    e.preventDefault();
    emit({
      vertices: vertices.filter((v: EditorGraphVertex) => v.id !== id),
      edges:    edges.filter((ed: EditorGraphEdge) => ed.source !== id && ed.target !== id),
    });
  ...
  function handleEdgeCtx(e: React.MouseEvent, idx: number) {
    e.preventDefault();
    emit({ edges: edges.filter((_: EditorGraphEdge, i: number) => i !== idx) });
  }
  ...
  return (
    <div className="space-y-2">
      {/* Title (bilingual) + directed toggle */}
      <div className="grid grid-cols-2 gap-2">
        <Input
          value={isUk ? graph.titleUk : graph.title}
          onChange={(e) => emit(isUk ? { titleUk: e.target.value } : { title: e.target.value })}
      ...
      <div className="rounded-md border-2 border-input overflow-hidden bg-muted/30">
        ...
        >
          <defs>
            <marker id="tg-arrow" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto">
              <path d="M0,0 L0,6 L8,3 z" className="fill-muted-foreground" />
            </marker>
          </defs>

          {edges.map((edge: EditorGraphEdge, i: number) => {
            ...
            const curved = directed && edges.some(
              (ed: EditorGraphEdge) => ed.source === edge.target && ed.target === edge.source,
            );
            if (curved) {
              const OFFSET = 28;
            ...
            return (
              <g key={i} onContextMenu={(e) => handleEdgeCtx(e, i)} style={{ cursor: 'context-menu' }}>
                <line x1={x1} y1={y1} x2={x2} y2={y2}
                  ...
                  markerEnd={directed ? 'url(#tg-arrow)' : undefined}
                />
          ...
          {vertices.map((v: EditorGraphVertex) => {
